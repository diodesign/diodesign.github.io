...
(async function () {
    ...
    function finalizeUI(showDisclaimer) {
        ...
        const resetLinkHtml = '<a href="#" class="new-search-link">Ask another question &rarr;</a>';
        if (showDisclaimer) {
            searchReset.innerHTML =
                '<p class="ai-disclaimer">This response was generated by an AI model in your browser, and may be inaccurate.</p>' +
                `<p>${resetLinkHtml}</p>`;
        } else {
            searchReset.innerHTML = `<p>${resetLinkHtml}</p>`;
        }

        if (searchQueryTitle) {
            searchQueryTitle.textContent = searchInput.value;
            searchQueryTitle.style.display = 'block';
        }
        searchInput.style.display = 'none';
        searchInput.disabled = true;
    }

    /** Resets the search UI smoothly in-place without page reload. */
    function resetSearch() {
        searchInput.value = '';
        if (searchQueryTitle) {
            searchQueryTitle.textContent = '';
            searchQueryTitle.style.display = 'none';
        }
        ...
        if (splashWrapper && splashText) {
            [splashWrapper, splashText].forEach(el => el.classList.remove('collapsed'));
            // Trigger canvas resize after the height transition finishes
            splashWrapper.addEventListener('transitionend', function onEnd(e) {
                if (e.propertyName === 'height') {
                    if (typeof resize === 'function') {
                        resize();
                    }
                    splashWrapper.removeEventListener('transitionend', onEnd);
                }
            });
        }

        searchInput.disabled = false;
        searchInput.placeholder = 'Ask another question…';
        searchInput.focus();
    }
    ...
    searchInput.addEventListener('input', () => {
        ...
        if (searchBox.classList.contains('complete')) {
